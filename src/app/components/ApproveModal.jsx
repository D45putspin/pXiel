...
export default function ApproveModal({ isOpen, onClose, onApprove, currentAllowance = 0, pricePerPaint = 1 }) {
    ...
    return (
        <div className="modal-backdrop" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
            <div className="pixel-frame" style={{ background: '#0a0b0f', border: '1px solid rgba(255,255,255,0.18)', width: 'min(520px, 92vw)', padding: 20, color: '#ffd1e0', boxShadow: '0 8px 40px rgba(0,0,0,0.5)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                    <h3 style={{ margin: 0, fontSize: 18 }}>Approve XIAN Allowance</h3>
                    <button className="btn" onClick={onClose} style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.18)', color: '#ffd1e0', padding: '6px 10px' }}>✕</button>
                </div>

                <p style={{ margin: '8px 0 14px', color: '#c9a8b4' }}>Grant the canvas contract permission to spend your XIAN for painting. You will only need to approve occasionally.</p>
                ...
                <div className="control-group" style={{ marginBottom: 12 }}>
                    ...
                    <div className="pill" style={{ display: 'inline-block', padding: '6px 10px', border: '1px solid rgba(255,255,255,0.12)' }}>
                        {currentAllowance} XIAN ({remainingPaints} paints available)
                    </div>
                </div>

                <div className="control-group" style={{ marginBottom: 12 }}>
                    <label className="label" style={{ display: 'block', marginBottom: 6 }}>Approve amount (XIAN)</label>
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        <input
                            ...
                            style={{ flex: 1, background: '#0e1016', color: '#ffd1e0', border: '1px solid rgba(255,255,255,0.12)', padding: '8px 10px' }}
                        />
                ...
                <p style={{ marginTop: 12, fontSize: 12, color: '#a07f8b' }}>Security tip: Only approve what you plan to use. You can re-approve later.</p>
            </div>
        </div>
    );
}
